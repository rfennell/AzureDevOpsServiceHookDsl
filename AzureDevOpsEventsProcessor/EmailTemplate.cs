...
namespace AzureDevOpsEventsProcessor
{
    ...
    public class EmailTemplate
    {
        ...
        public static string FindTemplate(string path, string workItemType)
        {
            ...
            {
                logger.Info(string.Format("AzureDevOpsEventsProcessor: Using template:{0}", path));
                if (System.IO.File.Exists(path) == false)
                {
                    logger.Error(string.Format("AzureDevOpsEventsProcessor: Cannot find template file [{0}]", path));
                    return string.Empty;
                }
                else
                {
            ...
            else
            {
                ...
                logger.Info(string.Format("AzureDevOpsEventsProcessor: Using template:{0}", fileName));
                if (System.IO.File.Exists(fileName) == false)
                {
                    logger.Error(string.Format("AzureDevOpsEventsProcessor: Cannot find template file [{0}] or work item name based template [{1}]", path, fileName));
                    return string.Empty;
                }
                else
                {
